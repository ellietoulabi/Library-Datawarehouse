-----------------###### DW Fact Dated Snap Lendings First SP ######---------------
USE LibraryDW
GO
CREATE OR ALTER PROCEDURE DW_Fact_Dated_Snap_Lendings_First_SP @end date
AS
BEGIN
	...
	set @start_date = Convert(date,
									CAST(
										(select min(DateID) from LendingsDataMart.transactionFactOverBorrowedBooks) as varchar),
							  112)
	set @end_date = @end
