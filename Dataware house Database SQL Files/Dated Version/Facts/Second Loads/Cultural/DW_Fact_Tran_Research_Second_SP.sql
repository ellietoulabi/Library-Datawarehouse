-----------------###### DW Fact Tran Research Second SP ######---------------
USE LibraryDW
GO
CREATE OR ALTER PROCEDURE DW_Fact_Tran_Research_Second_SP
AS
BEGIN
	IF OBJECT_ID('[LibraryDW].[dbo].[tranTmp]') IS NOT NULL
		BEGIN
			drop table 	[LibraryDW].[dbo].[tranTmp]
		END
		IF OBJECT_ID('[LibraryDW].[dbo].[tranTmp2]') IS NOT NULL
		BEGIN
			drop table 	[LibraryDW].[dbo].[tranTmp2]
		END
		IF OBJECT_ID('[LibraryDW].[dbo].[tranTmp3]') IS NOT NULL
		BEGIN
	...
	SET @start = (SELECT ISNULL(CONVERT(datetime, convert(varchar(10),MAX(DateID))),'2009-03-21 00:00:00.000') FROM [LibraryDW].[CulturalDataMart].[transactionFactOverResearch]);
