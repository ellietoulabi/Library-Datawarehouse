-----------------###### DW Fact Snap Salary Second SP ######---------------
USE LibraryDW
GO
CREATE OR ALTER PROCEDURE DW_Fact_Snap_Salary_Second_SP
AS
BEGIN
	...
	set @start_date = Convert(date,
									CAST(
										(select max(DateID) from FinancialDataMart.snapshotFactOverSalary) as varchar),
							  112)
	...
	begin
		set @start_date = DATEADD(YEAR, DATEDIFF(YEAR, 0, @start_date), 0)
